...
interface Phase5Props { project: Project; items: ContentItem[]; onDone: (items: ContentItem[]) => void; }
...
export default function Phase5Video({ project, items: init, onDone }: Phase5Props) {
  ...
  const startAllVideos = async () => {
    for (let i = 0; i < items.length; i++) {
      const st = states[items[i].id];
      if (st.mode !== 'none' && st.script.action && !st.videoUrl) {
        startVideo(items[i]);
  ...
  return (
    <div className="space-y-6 py-4">
      ...
      <div className="flex items-center gap-3 justify-center flex-wrap">
        <button onClick={generateAllScripts} className="btn btn-ghost text-sm">
          🤖 Alle Scripts per KI generieren
        </button>
        <button onClick={startAllVideos} className="btn btn-primary text-sm px-6">
